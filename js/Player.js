var Player = function (leftKey, rightKey, shootKey, jumpKey, meleeKey, useKey, moveSpeed, maxMoveSpeed, weight) {
	...
	data = new createjs.SpriteSheet({
		...
		frames: [
			...
			[0,0,78,106,6,3,9.9],[83,0,78,106,6,3,9.9],[166,0,78,106,6,3,9.9],[0,111,78,106,6,3,9.9],[83,111,78,106,6,3,9.9],[166,111,78,106,6,3,9.9],[0,222,78,106,6,3,9.9],[83,222,78,106,6,3,9.9]
		],
		animations: { 
			idle: [0, 10],
			run: [11, 27],
	...
	if (typeof actor !== 'undefined') {
		...
		if (intersection) {
			...
			if (this.hitTimer <= 0) {
				...
				this.image.addEventListener('animationend', function (event) {
					event.remove();
					event.currentTarget.gotoAndPlay('idle');
				});
			}
		}
